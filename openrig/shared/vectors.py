...
class Point(object):
    ...
    def distance(self, point):
        """Return the dis5tance between two points."""
        return math.sqrt((point.x - self.x)**2 + (point.y - self.y)**2 + (point.z - self.z)**2)

    def midpoint (self, point):
        """Return a Point instance as the midpoint between two points."""
        return Point((point.x + self.x)/2, (point.y + self.y)/2, (point.z + self.z)/2)
