...
def switch(paramNode, value):
    ...
    if value == 1:
        ...
        for node in ikMatchTransforms:
            matrixList.append(mc.xform(node, q=True, ws=True, rotation=True))
            ...
            if mc.objExists("%s.autoClav" % clavicleCtrl):
                for i in xrange(20):
                    ...
                    for matrix, ctrl in zip(matrixList, fkControls):
                        mc.xform(ctrl, ws=True, rotation=matrix)
                    # clav
    ...
    # To ik
        ...
        if is_colinear:
            ...
            pv_vec = getPoleVectorPosition(fkMatchTransforms, 20)
...
def isColinear(namespace, match_list, threshold=.008):
    ...
    if len(match_list) != 3:
        raise RuntimeError("{0} must be a lenght of three and a list.".format(match_list))
    mid_vec = om2.MVector(*mc.xform('{}:{}'.format(namespace, match_list[1]), q=True, ws=True, t=True))
    first_node_vec = om2.MVector(*'{}:'.format(namespace, match_list[0]), q=True, ws=True, t=True))
    last_node_vec = om2.MVector(*'{}:'.format(namespace, match_list[-1]), q=True, ws=True, t=True))
